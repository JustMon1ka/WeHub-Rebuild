...
- 本模块将负则实现用户的基本个人信息展示和关注、被关注、访客列表的查看功能。
- 本模块还将负则追踪用户习惯，记录用户浏览行为，提供用户等级与权限的查看接口。
...
| 用户悬浮卡片 | 鼠标悬浮时显示用户头像、昵称、关注状态等简要信息 |
| 用户推荐卡片 | 推荐多个受欢迎但未关注用户，显示头像、昵称等信息 |
...
- **用户主页面** 
    ...
    - 子页面：用户发布的帖子列表、回复列表、点赞列表、浏览记录列表（由其他组实现）

- **用户卡片悬浮**
    - 鼠标悬浮时显示小卡片，点击跳转至用户主页
    - 显示：用户头像、昵称、关注状态、个人简介等
...
1. 用户访问其他用户主页
2. 系统根据用户ID获取用户信息
3. 展示用户头像、昵称、个人简介、兴趣标签、关注数量、关注者数量、帖子数量、浏览记录等
4. 根据用户类型（自身、关注者、其他用户）展示不同的信息

**用户卡片悬浮流程**
1. 用户鼠标悬浮在其他用户头像上，系统将显示小卡片
2. 小卡片显示用户头像、昵称、关注状态、个人简介等
3. 用户点击小卡片跳转至对应用户主页
...
- **用户信息卡片（UserProfileCard.vue）**
    - 通过 props 接收用户信息数据，展示用户头像、昵称、个人简介等。
        - 点击编辑资料按钮时，触发事件跳转至个人信息页面。

- **用户卡片悬浮版本**
    - 鼠标悬浮时，显示小卡片组件，展示用户头像、昵称、关注状态等。
    - 点击小卡片时，跳转至对应用户主页。

- **用户推荐卡片**
    - 在用户主页底部展示推荐用户卡片，点击跳转至对应用户主页。
    - 推荐用户为受欢迎但未关注的用户。
