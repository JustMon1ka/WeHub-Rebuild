...
onMounted(() => {
  userInfo.value.loadUserData().catch(() => {
  })
})
  ...
  <article @click="goDetail"
    ...
    <div class="flex items-start gap-3">
      ...
      <router-link @click.stop :to="{ name:'UserPage', params:{ userId_p: String(post.userId) } }"
                   class="shrink-0">
        <img v-if="userInfo.avatarUrl" :src="userInfo.avatarUrl" class="w-16 h-16 rounded-full"
             alt="avatar">
        <PlaceHolder v-else width="80" height="80" :text="userInfo.nickname || userInfo.username"
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2 text-sm text-slate-400">
          <router-link @click.stop
                       :to="{ name:'UserPage', params:{ userId_p: String(post.userId) } }"
                       class="font-medium text-slate-200 hover:underline truncate max-w-[40%]">
            {{ userInfo.nickname || (post.userId) }}
          </router-link>
        ...
        <div class="mt-2 flex items-center gap-4 text-slate-500">
          ...
          <div v-if="post.tags?.length" class="hidden md:flex items-center gap-1 flex-wrap">
            <span v-for="t in post.tags" :key="t"
                  class="px-2 py-0.5 rounded-full bg-slate-800 text-slate-400 text-xs">#{{ t
              }}</span>
          </div>
        </div>
      </div>
