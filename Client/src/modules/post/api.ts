...
export async function toggleLike(data: ToggleLikeRequest) {
  const resp = await postHttp.post<BaseResp>("posts/like", data);
  return resp.data;
}

export async function toggleFavorite(data: { type: string; target_id: number; favorite: boolean }) {
  const resp = await postHttp.post<BaseResp>("posts/favorite", data);
  return resp.data;
}

export async function getMyFavorites() {
  const resp = await postHttp.get<FavoriteListResp>("posts/favorite");
  return resp.data;
}
...
export async function sharePost(targetId: number, comment: string): Promise<any> {
  const res = await postHttp.post("posts/share", { targetId, comment });
  return unwrap(res.data);
}

export const postService = {
  async getComments(postId: number): Promise<Comment[]> {
    const resp = await postHttp.get("posts/comments", { params: { post_id: postId }});
    const data = unwrap<any>(resp.data);
    return data.data || [];
  },

  async submitComment(commentData: CommentRequest): Promise<any> {
    const resp = await postHttp.post("posts/comment", commentData);
    return unwrap(resp.data);
  },

  async deleteComment(type: 'comment' | 'reply', targetId: number): Promise<boolean> {
    const resp = await postHttp.delete("posts/comment", { params: { type, target_id: targetId }});
    const data = unwrap<any>(resp.data);
